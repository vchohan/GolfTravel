package com.vchohan.golftravel.weather.widgets;
...
public abstract class AbstractWidgetProvider extends AppWidgetProvider {
    ...
    protected Weather parseWidgetJson(String result, Context context) {
        try {
            ...
            // Temperature
            float temperature = UnitConvertor
                .convertTemperature(Float.parseFloat(reader.optJSONObject("main").getString("temp").toString()), sp);
            ...
            double
                pressure =
                UnitConvertor
                    .convertPressure((float) Double.parseDouble(reader.optJSONObject("main").getString("pressure").toString()), sp);
            ...
            if (lastUpdateTimeInMillis < 0) {
                ...
                lastUpdate =
                    context
                        .getString(R.string.last_update_widget, WeatherActivity.formatTimeWithDayIfNotToday(context, lastUpdateTimeInMillis));
            }
            ...
            widgetWeather.setWind(context.getString(R.string.wind) + ": " + new DecimalFormat("#.0").format(wind) + " " +
                localize(sp, context, "speedUnit", "m/s")
                + (widgetWeather.isWindDirectionAvailable() ? " " + WeatherActivity.getWindDirectionString(sp, context, widgetWeather) : ""));
            widgetWeather.setPressure(context.getString(R.string.pressure) + ": " + new DecimalFormat("#.0").format(pressure) + " " +
                localize(sp, context, "pressureUnit", "hPa"));
            ...
            widgetWeather.setIcon(setWeatherIcon(
                Integer.parseInt(reader.optJSONArray("weather").getJSONObject(0).getString("id")),
                Calendar.getInstance().get(Calendar.HOUR_OF_DAY), context));
